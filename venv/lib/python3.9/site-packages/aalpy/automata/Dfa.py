...
class Dfa(DeterministicAutomaton[DfaState[InputType]]):
    ...
    def simulate(self, input_sequence):
        """
        Simulate the DFA with a given sequence of inputs.

        Args:
            input_sequence: A sequence of inputs to simulate on the DFA.

        Returns:
            True if the final state reached is an accepting state, False otherwise.
        """
        # Start from the initial state
        current_state = self.initial_state
        for input_symbol in input_sequence:
            if input_symbol in current_state.transitions:
                current_state = current_state.transitions[input_symbol]
            else:
                # If an input leads to no transition, return False (or handle as desired)
                return False
